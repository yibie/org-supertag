...
如果你把 `~/.emacs.d/org-supertag/`（或你配置的 `supertag-db-file` 所在目录）放进 Dropbox / iCloud / Syncthing 之类的同步文件夹，让它跟着你在多台机器间走，请先了解其中的取舍：
...
这只是权宜之计，不是最终方案——真正的多机同步需要一个懂"合并"的传输层（git 懂，文件同步盘不懂）。如果这正是你需要的，请留意后续即将推出的 git 原生同步模式；在此之前，上面的单写者纪律是安全使用同步文件夹的官方推荐做法。
