...
- Fixed =Symbol's value as variable is void: --cl-block-...--= errors
  ...
  =restore-heading-ids= script. A new repo-wide regression test
  (=test/cl-block-regression-test.el=, wired into =test/run-tests.sh=)
  locks the pattern down.
- Fixed deleted-file cleanup being permanently lost when destructive
  sync is disallowed: both =supertag-sync--check-and-sync-legacy= and
  =...-guarded= removed the file from sync state before the (guarded)
  orphan check ran, so its nodes were stranded with no retry. The
  state entry is now kept until destructive cleanup is allowed, and
  =supertag-sync--process-single-file= no longer persists the new
  content hash while node deletions are deferred, so the retry also
...
- Fixed =test/aggregate-test.el= being unloadable: a stray paren
  ...
  unbalanced top-level forms (caught by the new lint test).
