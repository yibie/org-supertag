...
*** =#tag= Completion
- Made the "create new tag" option actually visible in the popup. The
  previous attempt used =[Create New Tag: <name>]= as the candidate
  string, but orderless (corfu's default matcher) treated the bracket
  text as required-to-match and filtered the candidate out — the
  popup looked as if it offered no "new tag" path even though one
  existed. Switched to a two-part design:
  - candidate string = the bare prefix the user is typing (orderless
    cannot filter it because it matches itself exactly);
  - a metadata =affixation-function= renders =[Create New Tag]= as a
    suffix column next to the candidate, so the popup shows
    =thinking  [Create New Tag]= alongside the existing =[tag]=
    entries.
