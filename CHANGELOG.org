...
- Added =M-x supertag-view-stream=: choose a tag to read its nodes and all
  transitive =:extends= descendants as a creation-ordered full-body stream.  The
  default split layout pairs a title-only index with the main reading buffer;
  =s= toggles plain layout, =e= opens a source-backed narrowed indirect edit,
  and =v= opens Node View for field changes.  The View Runtime remains
