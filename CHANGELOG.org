...
** Features
*** Tag Discovery View
- Add tag discovery view to help users find and select tags
- Support tag filtering and sorting
- Display tag details including description and usage count
- Allow tag selection and additio
- Full keyboard driven operation interface:
  - ~a~ - Add new column
  - ~t~ - Add tag to column
  - ~d~ - Delete column
  - ~r~ - Reset view
...
*** Multi-column Tag View
- Add multi-column tag view to allow users to compare different tag combinations side by side:
- Use command ~M-x org-supertag-tag-columns~ to start multi-column tag comparison view
- Each column displays nodes符合特定标签组合
- Support dynamic addition and deletion of columns
- Support adding additional tag filtering conditions to columns
- Full keyboard driven operation interface:
  - ~a~ - Add new column
  - ~t~ - Add tag to column
  - ~d~ - Delete column
