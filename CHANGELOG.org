...
*** =#tag= Completion
- Restored the explicit =[Create New Tag: <name>]= candidate in the
  popup. The previous "use the typed text itself as the candidate"
  trick made the popup look as if it offered no way to create a new
  tag — users could not see whether typing more would commit a new
  tag or just keep filtering existing ones.
- The placeholder is rewritten to the bare =<name>= during commit
  (`post-completion-action'), so the buffer ends up with the real
  tag, not the bracketed label.
- Pinned the CAPF =completion-styles= metadata to =(basic substring
  partial-completion)= so orderless/corfu strict matchers do not
  filter the placeholder out when the user has only typed part of
  the new name.
- Typing =#newtag= and continuing to write (without explicitly
  selecting the popup candidate) still records the tag immediately
  via a new =post-self-insert-hook=. corfu/company quit their popup
  silently in that case and never call the CAPF =:exit-function=, so
  the tag used to be lost until the user re-triggered completion
  with =C-M-i=. The new hook fires only when the just-inserted
  character ends a =#tag= context, and skips tags already on the
  node.
...
- Added =test-completion-newtag.el= covering placeholder visibility,
  the four exit-status paths, the ignored-candidate hook path, the
  prose-no-tag path, and the duplicate-tag path.
