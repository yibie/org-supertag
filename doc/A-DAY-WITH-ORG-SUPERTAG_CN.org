...
#+begin_src emacs-lisp
  ;; 定义自定义仪表盘视图
  (define-supertag-view evening-review "晚间复盘仪表盘"
    (tag "task")
    ;; 区域 1：今天完成的任务
    (insert (supertag-view-header "✅ 今日已完成"))
    ...
    ;; 区域 2：明天的任务
    (insert (supertag-view-header "📅 明日待办"))
    (dolist (node (supertag-query
                   '(and (tag "task")
      ...
      (supertag-view-card node))
    ;; 区域 3：待读论文
    (insert (supertag-view-header "📚 阅读队列"))
    (dolist (node (supertag-query
                   '(and (tag "paper")
                         (field "status" "unread"))))
      (supertag-view-card node)))
...
| =M-x supertag-sync-full-rescan=         | 从文件重建数据库                 | 每月   |
